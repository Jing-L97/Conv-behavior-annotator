...
from .utilities import (
    BABYLM_DATA_DIR,
    BABYLM_DATA_DIR_CLEAN,
    BABYLM_DATA_PATH_DEV_CLEAN,
    CHILDES_LM_DATA_FILE,
    DEV_SET,
    SPEAKER_CODES_CAREGIVER,
    TRAIN_SET,
    TRAINING_TRACK_STRICT_SMALL,
)
...
def train_tokenizer(
    save_path, vocab_size, data_iterator=None, data_file_names=None, training_track=None, tokenizer_type="word_level"
):
    ...
    if tokenizer_type == "bpe":
        ...
        if data_iterator is not None:
            ...
            trainer = WordLevelTrainer(
                min_frequency=VOCAB_MIN_WORD_FREQ, special_tokens=SPECIAL_TOKENS, vocab_size=vocab_size
            )
        ...
        if data_iterator is not None:
            tokenizer.train_from_iterator(
                data_iterator, vocab_size=vocab_size, special_tokens=SPECIAL_TOKENS + [SEP_TOKEN, CLS_TOKEN]
            )
        else:
            paths = [os.path.join(BABYLM_DATA_DIR_CLEAN, training_track, f"{name}.train") for name in data_file_names]
            tokenizer.train(files=paths, vocab_size=vocab_size, special_tokens=SPECIAL_TOKENS)
...
def replace_contractions(words):
    words = [
        word if word.replace(",", "") not in CONTRACTIONS else CONTRACTIONS[word.replace(",", "")] for word in words
    ]
...
class ChildesDataModule(LightningDataModule):
    def __init__(
        self,
        lm_data_path=CHILDES_LM_DATA_FILE,
        ...
        if not os.path.isfile(train_data_path) or not os.path.isfile(val_data_path):
            ...
            data_train, data_val = train_test_split(
                data, test_size=DEV_SET_SIZE, shuffle=True, random_state=SPLIT_RANDOM_STATE
            )
        ...
        if not (
            os.path.isfile(os.path.join(tokenizer_dir, "vocab.json"))
            or os.path.isfile(os.path.join(tokenizer_dir, "vocab.txt"))
            or os.path.isfile(os.path.join(tokenizer_dir, "tokenizer.json"))
        ):
        ...
        self.dataset_dev = ChildesLMDataset(
            data_val, tokenizer=self.tokenizer, tokenizer_type=tokenizer_type, max_len=max_len
        )
        self.dataset_train = ChildesLMDataset(
            data_train, tokenizer=self.tokenizer, tokenizer_type=tokenizer_type, max_len=max_len
        )
    ...
    def val_dataloader(self):
        lm_dataloader = DataLoader(
            self.dataset_dev, batch_size=self.batch_size, num_workers=self.num_workers, collate_fn=self.collate_fn
        )
        return lm_dataloader
...
class BabyLMDataModule(LightningDataModule):
    def __init__(
        self,
        training_track=TRAINING_TRACK_STRICT_SMALL,
        vocab_size=10000,
        max_len=DEFAULT_MAX_LEN,
        batch_size=128,
        num_workers=4,
        ...
        self.dataset_dev = BabyLMDataset(
            BABYLM_DATA_PATH_DEV_CLEAN,
        ...
        self.dataset_train = BabyLMDataset(
            data_path_train, tokenizer=self.tokenizer, max_len=max_len, data_file_names=data_file_names, split="train"
        )
...
def compute_reward_value(utt, reward_cr=0, reward_ack=1, reward_other=0.5):
    if utt.response_is_clarification_request:
        return reward_cr
    if utt.response_is_acknowledgement:
        return reward_ack
    return reward_other
...
class FeedbackDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        encoded_sample = self.tokenizer(
            item["utt_transcript_clean"], add_special_tokens=True, truncation=True, max_length=self.max_len - 2
        )
