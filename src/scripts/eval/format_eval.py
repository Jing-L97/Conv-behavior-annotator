...
def restructure_directories(base_path, new_dir_name, dry_run=False):
    """For every result_best_reward.csv and utt_best_reward.csv found under base_path,
    insert a new subdirectory (new_dir_name) between the parent folder and the CSV files.

    Example:
      Before: .../topline_.../result_best_reward.csv
      After:  .../topline_.../<new_dir_name>/result_best_reward.csv

    """
    target_files = {"result_best_reward.csv", "utt_best_reward.csv"}
    moved = 0
    skipped = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("base_path", help="Root path to walk (e.g. /scratch2/jliu/Feedback/results/ppo)")
