...
from pkg.rlhf.utilities import (
    ...
    CKPT_DIR_BEST_BLIMP,
    CKPT_DIR_BEST_REWARD,
    CKPT_DIR_BEST_VAL_LOSS,
    CKPT_DIR_BEST_ZORRO,
...
class ChildesPPOTrainer(PPOTrainer):
    def __init__(
        ...
        model: PreTrainedModelWrapper | None = None,
        ref_model: PreTrainedModelWrapper | None = None,
        tokenizer: PreTrainedTokenizerBase | None = None,
        dataset: torch.utils.data.Dataset | Dataset | None = None,
        optimizer: torch.optim.Optimizer | None = None,
        num_shared_layers: int | None = None,
        lr_scheduler: torch.optim.lr_scheduler._LRScheduler | None = None,
        training_data_collator: typing.Callable | None = None,
    ):
        def collator(data):
        ...
        super(ChildesPPOTrainer, self).__init__(
            config,
            model,
            ref_model,
            tokenizer,
    ...
    def step(
        ...
        response_masks: list[torch.LongTensor] | None = None,
        lm_inputs: list[torch.LongTensor] | None = None,
    ...
    def train_minibatch(
        ...
        """
        self.model.train()
        loss, train_stats = self.loss(old_logprobs, values, logits, vpreds, logprobs, mask, advantages, returns)
        if lm_loss_coef > 0:
    ...
    def loss(
        ...
        stats = dict(
            loss=dict(
                policy=pg_loss.detach(), value=vf_loss.detach(), entropy=entropy_loss.detach(), total=loss.detach()
            ),
            policy=dict(
    ...
    def log_stats(
        ...
        if self.accelerator.is_main_process:
            ...
            # Log stats
            if "query" not in batch and "utterance" not in batch:
                ...
                table_rows = [list(r) for r in zip(*batch_list, rewards.cpu().tolist(), strict=False)]
...
def build_policy_trainer_datasets(
    data_path, lm_val_data_path, tokenizer, query_max_length, utt_max_length=DEFAULT_MAX_LEN
):
...
def eval(model, tokenizer, config, trainer, ckpt_dir, final=False):
    ...
    results_babylm = eval_babylm(
        model, tokenizer, ckpt_dir=ckpt_dir, ppo_trainer=trainer, device=trainer.accelerator.device.index, config=config
    )
...
def compute_rewards(
    ...
    if value_model is None:
        ...
        texts_encoded = value_model_tokenizer(
            utterances, padding=True, truncation=True, return_tensors="pt", max_length=output_max_length + 10
        )
        with torch.no_grad():
...
def main():
    ...
    train_dataset, lm_val_dataset = build_policy_trainer_datasets(
        ...
        lm_val_data_path=config.lm_val_data_path,
        tokenizer=tokenizer,
    ...
    def generate(batch, query_length_sampler, use_queries):
        ...
        batch["utterance"] = [
            tokenizer.decode(torch.cat((q, r)), skip_special_tokens=True)
            for q, r in zip(query_tensors, response_tensors, strict=False)
        ]
    ...
    while step <= config.steps:
        ...
        for batch in tqdm(ppo_trainer.dataloader):
            ...
            stats = ppo_trainer.step(
                query_tensors, response_tensors, rewards, lm_inputs=batch["input_ids"], lm_loss_coef=config.lm_loss_coef
            )
