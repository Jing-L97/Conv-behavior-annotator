...
from pkg.rlhf.data import SEQUENCE_START_TOKEN, ChildesDataModule
from pkg.rlhf.model import ChildesGPT
from pkg.rlhf.utilities import parse_babylm_metrics_results
    ...
    def generate_sample_sentences(self):
        ...
        for prefix in generation_prefixes:
            ...
            for step in range(10):
                if self.model_family == "causal":
                    inputs = tokenizer(
                        sequence, return_tensors="pt", add_special_tokens=False, return_token_type_ids=False
                    ).to(device)
                else:
...
def cli_main():
    checkpoint_callback = ModelCheckpoint(
        monitor="val_loss", mode="min", save_last=False, filename="{epoch:02d}-{val_loss:.2f}"
    )
    early_stop_callback = EarlyStopping(monitor="val_loss", patience=10, verbose=True, mode="min", min_delta=0.01)
    LightningCLI(
        BabyLMModel,
        ChildesDataModule,
