...
class CFRewardDataCollatorWithPadding:
    r"""Reward DataCollator class that pads the inputs to the maximum length of the batch.
...
class WandbPredictionProgressCallback(WandbCallback):
    ...
    def __init__(self, trainer, tokenizer, val_dataset, num_samples=100, freq=50):
    ...
    for fb_data_path in fb_data_paths:
        if os.path.isfile(fb_data_path):
            ...
            if fb_data_path.endswith(".csv"):
                ...
                with open(fb_data_path) as f:
                    data = f.read().splitlines()
                data = pd.DataFrame({"transcript_clean": data, reward_column_name: [1] * len(data)})
            else:
                raise RuntimeError(f"Unknown data format: {fb_data_path}")
        else:
            data = []
