...
def compute_scores_gec(
    ...
    tokenized_sentences = gec_model_tokenizer(
        utterances_gec, max_length=max_length, truncation=True, padding="max_length", return_tensors="pt"
    ).to(device)
    with torch.no_grad():
    ...
    scores = np.array(
        [
            clean_utt(utt) == clean_utt(utt_corrected)
            for utt, utt_corrected in zip(utterances, corrected_sentences, strict=False)
        ]
    ).astype(int)
    return scores
...
def compute_scores(
    ...
    utterances = [
        utt for utt, utt_len in zip(utterances, utt_lengths, strict=False) if utt_len > DEFAULT_MIN_GENERATION_LEN
    ]

    utts_finished = [tokenizer.eos_token_id in utt for utt in batch["utts"]]
    utterances = [utt for utt, utt_finished in zip(utterances, utts_finished, strict=False) if utt_finished]

    if len(utterances) == 0:
        return [], []

    scores_gec = compute_scores_gec(utterances, gec_model, gec_model_tokenizer)
    scores_childes_grammar = compute_scores_childes_grammaticality(
...
def eval_grammaticality_produced_utts(
    ...
    for i in range(num_batches):
        ...
        scores_childes_grammar, scores_gec, utterances = compute_scores(
            batch, childes_grammar_model, childes_grammar_model_tokenizer, gec_model, gec_model_tokenizer, tokenizer
        )
            ...
            sample_df = pd.DataFrame.from_dict(
                {
                    ...
                    "score_childes_grammaticality": scores_childes_grammar,
