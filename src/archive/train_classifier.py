...
from pkg.classifier.data import preprocess_childes_utterance
...
class ClassifierDataCollatorWithPadding:
    r"""DataCollator class that pads the inputs to the maximum length of the batch.
...
class WandbPredictionProgressCallback(WandbCallback):
    ...
    def __init__(self, trainer, tokenizer, val_dataset, num_samples=100, freq=50, target_column=None):
    ...
    def on_evaluate(self, args, state, control, **kwargs):
        ...
        if state.global_step % self.freq == 0:
            ...
            table["text"] = [
                utt + " " + resp
                for utt, resp in zip(
                    self.sample_dataset["utt_transcript_clean"],
...
class ClassifierTrainer(RewardTrainer):
    def __init__(
        self,
        ...
        data_collator = ClassifierDataCollatorWithPadding(tokenizer, max_length=max_length, target_column=target_column)
...
def build_classifier_datasets(data_path, target_column):
    ...
    data_train, data_test = train_test_split(
        data, test_size=TEST_SET_SIZE, shuffle=True, random_state=SPLIT_RANDOM_STATE
    )
...
def main():
    ...
    # put the model to the available device
    device = settings.get_torch_device()
    model.to(device)

    raw_datasets = build_classifier_datasets(settings.PATH.dataset_root / args.data_path, args.target_column)

    def preprocess_function(samples):
        texts = [
            utt + tokenizer.sep_token + resp
            for utt, resp in zip(samples["utt_transcript_clean"], samples["response_transcript_clean"], strict=False)
        ]
